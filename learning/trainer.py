...
def train_new_model(db_path):
    ...
    X_train, X_test, y_train, y_test, w_train, w_test = train_test_split(
        X, y, sample_weights, test_size=0.2, random_state=42, stratify=y if len(set(y)) > 1 else None
    )
    ...
    sklearn_path = f'models/intent_classifier_{model_version}.joblib'
    ...
    try:
        ...
        coreml_path = f'models/model_{model_version}.mlmodel'
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?)
        ''', (
            model_version, coreml_path, float(accuracy), len(X_train), datetime.now().isoformat()
        ))
        conn.commit()
        conn.close()
